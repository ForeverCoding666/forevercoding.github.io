$(function () {
    let playerContent2 = $('#player-content2') //长短变化
    let playerContent1 = $('#player-content1');// 歌曲信息模块部分dom元素
    let musicName = $('.music-name');          // 歌曲名部分dom元素 
    let artistName = $('.artist-name');        // 歌手名部分dom元素

    let musicImgs = $('.music-imgs');          // 左侧封面图dom元素

    let playPauseBtn = $('.play-pause');       // 播放/暂停按钮 dom元素
    let playPrevBtn = $('.prev');              // 上一首按钮 dom元素
    let playNextBtn = $('.next')               // 下一首按钮 dom元素

    let time = $('.time');                     // 时间信息部分 dom元素
    let tProgress = $('.current-time');        // 当前播放时间文本部分 dom元素
    let totalTime = $('.total-time');          // 歌曲总时长文本部分 dom元素

    let sArea = $('#s-area');                  // 进度条部分
    let insTime = $('#ins-time');              // 鼠标移动至进度条上面，显示的信息部分
    let sHover = $('#s-hover');                // 鼠标移动至进度条上面，前面变暗的进度条部分
    let seekBar = $('#seek-bar');              // 播放进度条部分
    ...
    let musicImgsData = ['img/bg.jpg', 'img/bg1.jpg', 'img/bg2.jpg']    // 图片地址数组
    let musicNameData = ['带我去找夜生活', '爱人错过', '唯一'];                   // 歌曲名数组
    let artistNameData = ['告五人', '告五人', '告五人']            // 创作歌手数组
    let musicUrls = ['mp3/music1.mp3', 'mp3/music2.mp3', 'mp3/music3.mp3'];// 歌曲mp3数组
    let currIndex = -1;              // 当前播放索引

    let buffInterval = null          // 初始化定时器 判断是否需要缓冲
    let len = musicNameData.length;  // 歌曲长度
    ...
    function playPause() {
        if (audio.paused) {
            playerContent2.addClass('active')//播放器长短变化
            ...
            playerContent2.removeClass('active')//播放器长短变化
